...
Markov Chain which its stationary distribution is the one we want to sample
from. 

The chain is generated by proposing candidates from another distribution
\\(q\\) and then accept it or reject it according to a probability
\\(\alpha\\).
...
\\(i \neq j\\) the probability of jump from \\(i\\) to \\(j\\) can be interpreted
